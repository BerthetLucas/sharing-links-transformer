...
test('When I share a valid Deezer link, I should get a Spotify Link', async ({ page }) => {
  ...
  await expect(page.getByText('You can now share this link to a Spotify User')).toBeVisible({ timeout: 20000 });
  await expect(page.getByText('https://open.spotify.com/track/4CeeEOM32jQcH3eN9Q2dGj')).toBeVisible({ timeout: 20000 });
});
